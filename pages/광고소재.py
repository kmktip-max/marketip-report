...
def _build_prompt(inp):
    ...
    meta_rule = (
        "\n━━━ 메타광고 핵심 원칙 ━━━\n"
        "[첫 줄이 전부다]\n"
        "사용자는 스크롤 중 광고를 보기 때문에 첫 1~2줄에서 스크롤을 멈추게 해야 한다.\n"
        "첫 문장에는 반드시 아래 중 최소 하나가 들어가야 한다:\n"
        "  공감 / 문제 / 공포 / 손실회피 / 호기심\n\n"
        "[첫 줄 예시 — 이 방향으로 작성]\n"
        "  광고비만 쓰고 문의가 없으신가요?\n"
        "  아직도 혼자 이혼 준비하고 계신가요?\n"
        ...
        "  1. 첫 줄 후킹 (스크롤 멈춤)\n"
        "  2. 문제 공감 (지금 처한 상황 묘사)\n"
        "  3. 손실/불안 요소 (대응 안 하면 어떻게 되는가)\n"
        "  4. 해결 가능성 (우리가 해결할 수 있다)\n"
        "  5. 신뢰 요소 (숫자, 경험, 사례)\n"
        "  6. CTA (자연스럽게 유도)\n\n"
        "[문체 규칙]\n"
        "  ✅ 모바일 가독성 우선 — 짧은 문장, 줄바꿈 적극 활용\n"
        "  ✅ 첫 2줄에 모든 집중\n"
        "  ❌ 설명형 문체 금지 (서비스를 설명하는 식)\n"
        "  ❌ 기업 소개·브로셔 느낌 금지\n"
        "  ❌ 추상적 브랜드 소개 금지\n"
        "  ❌ '전문적인 서비스를 제공합니다' 같은 표현 절대 금지\n"
        "  ❌ '고객 만족을 위해 노력합니다' 같은 표현 절대 금지\n"
    ) if has["meta"] else ""
    ...
    if has["meta"]:
        sections.append(
            ...
            '  body_copy(6~8줄. 반드시 위 [본문 구조] 6단계 순서로 작성. 줄바꿈 \\n 사용.\n'
            '    첫 줄은 반드시 스크롤을 멈추는 후킹 문장. 설명형·기업소개형 절대 금지),\n'
            '  title(광고 제목 — 짧고 강하게)\n'
            ']'
        )
    else:
...
{naver_rule}{shopping_rule}{google_rule}{meta_rule}
━━━ 검색광고 카피 핵심 원칙 ━━━
...
def _generate(client, inp):
    sys_msg = (
        "당신은 10년 이상 경력의 퍼포먼스 마케팅 전문가이자 소비자 심리 분석 전문가입니다. "
        "검색광고 카피는 '짧은 시간 안에 클릭을 유도'하는 것이 핵심입니다. "
        "메타(인스타/페이스북) 광고는 '첫 1~2줄 후킹'이 전부입니다 — "
        "스크롤 중인 사용자를 멈추게 할 공감·공포·손실회피·호기심을 첫 문장에 담아야 합니다. "
        "메타 본문은 반드시 후킹→문제공감→손실불안→해결가능성→신뢰요소→CTA 순서로 작성합니다. "
        "추상적 브랜드 소개, 설명형 문체, 기업 브로셔 느낌의 표현은 어떤 플랫폼에서도 절대 사용하지 않습니다. "
        "평범한 브랜드 문구, 누구나 쓰는 표현은 절대 사용하지 않습니다. "
        "모든 카피는 짧고 강하며 실전에서 바로 쓸 수 있어야 합니다. "
        "반드시 JSON으로만 응답하세요."
    )
