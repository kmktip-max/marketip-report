...
def _build_prompt(inp):
    ...
    if has["naver_search"]:
        sections.append(
            '"naver_search": [\n'
            '  4세트 (소비자문제해결형 / 서비스강점형 / 신뢰강조형 / 클릭유도형)\n'
            '  각 세트 필드: type, title(15자이내), description(45자이내), '
            'additional_title(15자이내), additional_description(45자이내), promo_text(14자이내)\n'
            ']'
        )
    else:
...
[마케팁 스타일 원칙]
- 클릭만 유도하는 문구 금지 → 실제 문의/전환 중심
- 소비자 불안·고통 공감 후 해결책 제시
- 손실 회피 심리 적극 활용
- 짧고 강한 문구 우선
- 예시 관점:
  "광고비만 쓰고 문의가 없다면 구조 문제입니다"
  "클릭은 나오는데 왜 전환이 안 될까요?"
  "고객은 많은데 문의가 없다면 랜딩 문제입니다"

[플랫폼별 전략]
네이버 검색: 검색 의도 중심, 글자수 엄수, 설명은 45자 최대 활용
메타/인스타: 첫 줄 강한 후킹, 모바일 최적화, 스크롤 멈추는 첫 문장
카카오: 친근한 톡 말투, 생활밀착형
구글: 검색 목적 기반, 명확한 USP 강조
당근/기타: 동네 밀착형 공감, 가까운 거리감

반드시 아래 JSON 구조로만 응답하세요:
{{
  "analysis": {{
    "consumer_insight": "소비자 심리 및 핵심 니즈 분석 3~4문장",
    "pain_points": ["불안요소1", "불안요소2", "불안요소3"],
    "conversion_points": ["전환포인트1", "전환포인트2"],
    "platform_strategy": "선택 플랫폼별 전략 방향 2~3문장"
  }},
  {sections_str}
}}
"""
...
def _generate(client, inp):
    sys_msg = (
        ...
        "광고 플랫폼 성격과 업종 특성을 깊이 이해하며, "
        "소비자가 클릭/문의/구매하도록 만드는 후킹형 광고 소재를 제작합니다. "
        "단순한 예쁜 문구가 아닌, 실제 전환을 만들어내는 실무형 광고 소재를 생성하세요. "
        "반드시 JSON으로만 응답하세요."
    )
