...
COLS_DISPLAY  = ["keyword","biz_name","is_ad","page","landing_url","domain","email","phone","memo"]
COL_RENAME    = {
    "keyword":"수집키워드","biz_name":"업체명","is_ad":"광고여부",
    "page":"노출페이지","landing_url":"랜딩URL","domain":"도메인",
...
def _to_xlsx(rows):
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        df[cols].to_excel(w, index=False, sheet_name="파워링크DB")
    return buf.getvalue()
...
1. `2` 선택 → 파워링크 DB 추출
2. 키워드 입력 (쉼표 또는 줄바꿈)
3. 지역 입력 (선택, 예: 서울 / 비워두면 전국)
4. 수집 페이지 수 입력
5. 실제 크롬 브라우저가 열리면서 자동 수집
6. 완료 후 Supabase 자동 저장 → 이 페이지에서 조회 가능
...
with st.expander("🔍 검색 · 필터", expanded=True):
    ...
    with f1:
        q_name = st.text_input("업체명 검색", key="f_biz")
    with f2:
...
filtered = results
if q_name:
    filtered = [r for r in filtered if q_name.lower() in (r.get("biz_name","") or "").lower()]
if q_kw:
...
edited = st.data_editor(
    ...
    column_config={
        ...
        "랜딩URL":  st.column_config.LinkColumn(disabled=True),
...
if edited is not None:
    orig_idx = {(r.get("domain") or r.get("landing_url","")): i for i, r in enumerate(results)}
    for df_idx, row in edited.iterrows():
        if df_idx < len(filtered):
            k = filtered[df_idx].get("domain") or filtered[df_idx].get("landing_url","")
            if k in orig_idx:
    ...
    if st.button("🗑️ 선택 삭제", use_container_width=True, key="pl_del"):
        if edited is not None:
            ...
            keep = [r for r in results
                    if (r.get("domain") or r.get("landing_url","")) not in del_keys]
            st.session_state[SESSION_KEY] = keep
            sb_save("sales_leads_powerlink", keep, FALLBACK_JSON)
            st.rerun()
with a2:
    if st.button("💾 저장", use_container_width=True, key="pl_save"):
        sb_save("sales_leads_powerlink", results, FALLBACK_JSON)
        st.success("저장 완료")
with a3:
    st.download_button("📥 CSV", _to_csv(filtered), "파워링크DB.csv",
                       "text/csv", use_container_width=True, key="pl_csv")
with a4:
    st.download_button("📊 XLSX", _to_xlsx(filtered), "파워링크DB.xlsx",
                       "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                       use_container_width=True, key="pl_xlsx")
