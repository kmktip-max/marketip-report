...
**① 광고 계정 등록**
**[계정 관리]** 탭 → **[+ 계정 추가]** 클릭 → 네이버 검색광고 API 키·Secret Key·Customer ID 입력 후 저장
(API 키는 네이버 검색광고 시스템 → 내 정보 → API 관리에서 발급)

**② 그룹 생성**
**[그룹 관리]** 탭 → **[+ 그룹 추가]** 클릭 → 그룹명·광고그룹 ID·목표순위·입찰가 범위·검색 도메인 입력
→ 검색 도메인: 광고 랜딩페이지 도메인 입력 (예: `admarketip.com`) — 이 설정이 없으면 순위 조회 불가

**③ 키워드 불러오기**
**[키워드 관리]** 탭 → 그룹 선택 → **[키워드 불러오기]** 클릭 → Naver API에서 키워드 자동 가져옴

**④ 자동입찰 시작**
**[입찰 현황]** 탭 → 그룹 카드의 **[자동입찰 포함]** 토글 ON → **[자동입찰 시작]** 버튼 클릭

**⑤ 자동 조정 확인**
스케줄러가 2분마다 현재 순위를 확인하고 목표순위 기준으로 입찰가를 자동으로 올리거나 낮춥니다
결과는 **실행 이력** 에서 30초마다 자동 갱신됩니다

> 💡 **팁** : 목표순위는 1~15위 중 광고 효율이 좋은 순위로 설정하세요. 보통 3~5위 권장.
> 파워링크 구좌가 없는 키워드(순위 조회 불가)는 자동으로 최소입찰가로 유지됩니다.
    """)
