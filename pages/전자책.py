"""전자책·강의 + 세팅/대행 상품 스토어 — 사이트 이용 → 노하우·서비스 구매 수익화 퍼널"""
...
def _ebook_card(grad, step, name, price, feats):
    ...
    return f"""
<div style="background:#fff;border:1.5px solid #E5E8ED;border-radius:16px;overflow:hidden;height:100%;
...
st.markdown("""
<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;padding:14px 18px;margin-top:14px;">
...
def _svc_card(badge, badge_bg, badge_fg, name, price, sub, feats, highlight=False):
    ...
    fh = "".join(f'<div style="font-size:12.5px;line-height:1.5;color:#374151;margin-bottom:6px;">✓ {f}</div>' for f in feats)
    return f"""
<div style="background:#fff;border:{border};border-radius:16px;padding:22px 20px;height:100%;">
...
""", unsafe_allow_html=True)
if st.button("💸  광고비 페이백 신청하러 가기", use_container_width=False, type="primary"):
    try:
...
st.markdown("<h3 style='font-weight:900;color:#111827;'>📨 구매 · 상담 신청</h3>", unsafe_allow_html=True)
st.markdown("""
<div style="background:#F8FAFC;border:1px solid #E5E8ED;border-radius:12px;padding:14px 18px;margin-bottom:14px;">
  <span style="font-size:13.5px;color:#334155;">💳 결제는 <b>계좌이체</b>로 진행됩니다. 아래로 신청을 남겨주시면
  <b>입금 계좌와 함께 1:1로 안내</b>해 드립니다. (무상 점검은 입금 없이 바로 진행)</span>
</div>
""", unsafe_allow_html=True)

_PRODUCTS = [
    "전자책 - 구조이해(STEP1)", "전자책 - 기초세팅(STEP2)", "전자책 - 심화세팅(STEP3)",
    "전자책 - 올인원 패키지",
    "세팅 - 광고 무상점검(무료)", "세팅 - 초기 광고세팅(신규)", "세팅 - 광고최적화세팅(기존)",
    "대행 - 성과보장 광고 운영대행", "기타 상담",
]
...
with _fcol, st.form("ebook_inquiry", clear_on_submit=True):
    ...
    q_product = st.selectbox("신청 상품", _PRODUCTS)
    q_memo  = st.text_area("문의 내용 (선택)", placeholder="궁금한 점이나 현재 광고 상황을 적어주세요.", height=80)
    if st.form_submit_button("신청하기", type="primary", use_container_width=True):
        if not q_name.strip() or not q_phone.strip():
            ...
            leads.append({
                ...
                "interest": q_product, "memo": q_memo.strip(),
                "from_user": _username or "비로그인",
                "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "status": "신규",
            })
            _save_leads(leads)
            st.success("신청이 접수되었습니다. 입금 계좌와 함께 빠르게 안내드리겠습니다! 🙏")
